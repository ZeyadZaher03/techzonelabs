<!DOCTYPE html>
<html lang="en">
	<head>
		<?php include "./includes/head.php"?>
		<title>TechZoneLabs</title>
	</head>
	<body>
		<?php include "./includes/navigation.php"?>

		<header class="header-section">
			<div class="header-section_container">
				<h1 class="hero-heading">BRING YOU BUSSNIESS TO LIVE</h1>
				<p class="heading-par">
					Aiming to be your Tech-Arm and success partner, writing
					memorable stories along the journey
				</p>
				<a class="button button-main button-hero" href="#">Start Now</a>
			</div>
		</header>

		<section class="section-service odd-section">
			<h2 class="section-header">Our Services</h2>
			<div class="services">
				<div class="services-container grid-three">
					<div class="service-item">
						<div class="service-item-icon">
							<svg
								version="1.1"
								id="android"
								xmlns="http://www.w3.org/2000/svg"
								xmlns:xlink="http://www.w3.org/1999/xlink"
								x="0px"
								y="0px"
								viewBox="0 0 553.048 553.048"
								style="
									enable-background: new 0 0 553.048 553.048;
								"
								xml:space="preserve"
							>
								<g>
									<g>
										<path
											d="M76.774,179.141c-9.529,0-17.614,3.323-24.26,9.969c-6.646,6.646-9.97,14.621-9.97,23.929v142.914
										c0,9.541,3.323,17.619,9.97,24.266c6.646,6.646,14.731,9.97,24.26,9.97c9.522,0,17.558-3.323,24.101-9.97
										c6.53-6.646,9.804-14.725,9.804-24.266V213.039c0-9.309-3.323-17.283-9.97-23.929C94.062,182.464,86.082,179.141,76.774,179.141z"
										/>
										<path
											d="M351.972,50.847L375.57,7.315c1.549-2.882,0.998-5.092-1.658-6.646c-2.883-1.34-5.098-0.661-6.646,1.989l-23.928,43.88
										c-21.055-9.309-43.324-13.972-66.807-13.972c-23.488,0-45.759,4.664-66.806,13.972l-23.929-43.88
										c-1.555-2.65-3.77-3.323-6.646-1.989c-2.662,1.561-3.213,3.764-1.658,6.646l23.599,43.532
										c-23.929,12.203-42.987,29.198-57.167,51.022c-14.18,21.836-21.273,45.698-21.273,71.628h307.426
										c0-25.924-7.094-49.787-21.273-71.628C394.623,80.045,375.675,63.05,351.972,50.847z M215.539,114.165
										c-2.552,2.558-5.6,3.831-9.143,3.831c-3.55,0-6.536-1.273-8.972-3.831c-2.436-2.546-3.654-5.582-3.654-9.137
										c0-3.543,1.218-6.585,3.654-9.137c2.436-2.546,5.429-3.819,8.972-3.819s6.591,1.273,9.143,3.819
										c2.546,2.558,3.825,5.594,3.825,9.137C219.357,108.577,218.079,111.619,215.539,114.165z M355.625,114.165
										c-2.441,2.558-5.434,3.831-8.971,3.831c-3.551,0-6.598-1.273-9.145-3.831c-2.551-2.546-3.824-5.582-3.824-9.137
										c0-3.543,1.273-6.585,3.824-9.137c2.547-2.546,5.594-3.819,9.145-3.819c3.543,0,6.529,1.273,8.971,3.819
										c2.438,2.558,3.654,5.594,3.654,9.137C359.279,108.577,358.062,111.619,355.625,114.165z"
										/>
										<path
											d="M123.971,406.804c0,10.202,3.543,18.838,10.63,25.925c7.093,7.087,15.729,10.63,25.924,10.63h24.596l0.337,75.454
										c0,9.528,3.323,17.619,9.969,24.266s14.627,9.97,23.929,9.97c9.523,0,17.613-3.323,24.26-9.97s9.97-14.737,9.97-24.266v-75.447
										h45.864v75.447c0,9.528,3.322,17.619,9.969,24.266s14.73,9.97,24.26,9.97c9.523,0,17.613-3.323,24.26-9.97
										s9.969-14.737,9.969-24.266v-75.447h24.928c9.969,0,18.494-3.544,25.594-10.631c7.086-7.087,10.631-15.723,10.631-25.924V185.45
										H123.971V406.804z"
										/>
										<path
											d="M476.275,179.141c-9.309,0-17.283,3.274-23.93,9.804c-6.646,6.542-9.969,14.578-9.969,24.094v142.914
										c0,9.541,3.322,17.619,9.969,24.266s14.627,9.97,23.93,9.97c9.523,0,17.613-3.323,24.26-9.97s9.969-14.725,9.969-24.266V213.039
										c0-9.517-3.322-17.552-9.969-24.094C493.888,182.415,485.798,179.141,476.275,179.141z"
										/>
									</g>
								</g>
								<g></g>
								<g></g>
								<g></g>
								<g></g>
								<g></g>
								<g></g>
								<g></g>
								<g></g>
								<g></g>
								<g></g>
								<g></g>
								<g></g>
								<g></g>
								<g></g>
								<g></g>
							</svg>
						</div>
						<h2 class="service-item-heading">
							android development
						</h2>
					</div>
					<div class="service-item">
						<div class="service-item-icon">
							<svg
								version="1.1"
								id="apple"
								xmlns="http://www.w3.org/2000/svg"
								xmlns:xlink="http://www.w3.org/1999/xlink"
								x="0px"
								y="0px"
								viewBox="0 0 512.003 512.003"
								style="
									enable-background: new 0 0 512.003 512.003;
								"
								xml:space="preserve"
							>
								<g>
									<g>
										<path
											d="M351.98,0c-27.296,1.888-59.2,19.36-77.792,42.112c-16.96,20.64-30.912,51.296-25.472,81.088
										c29.824,0.928,60.64-16.96,78.496-40.096C343.916,61.568,356.556,31.104,351.98,0z"
										/>
									</g>
								</g>
								<g>
									<g>
										<path
											d="M459.852,171.776c-26.208-32.864-63.04-51.936-97.824-51.936c-45.92,0-65.344,21.984-97.248,21.984
										c-32.896,0-57.888-21.92-97.6-21.92c-39.008,0-80.544,23.84-106.88,64.608c-37.024,57.408-30.688,165.344,29.312,257.28
										c21.472,32.896,50.144,69.888,87.648,70.208c33.376,0.32,42.784-21.408,88-21.632c45.216-0.256,53.792,21.92,87.104,21.568
										c37.536-0.288,67.776-41.28,89.248-74.176c15.392-23.584,21.12-35.456,33.056-62.08
										C387.852,342.624,373.932,219.168,459.852,171.776z"
										/>
									</g>
								</g>
								<g></g>
								<g></g>
								<g></g>
								<g></g>
								<g></g>
								<g></g>
								<g></g>
								<g></g>
								<g></g>
								<g></g>
								<g></g>
								<g></g>
								<g></g>
								<g></g>
								<g></g>
							</svg>
						</div>
						<h2 class="service-item-heading">
							ios development
						</h2>
					</div>
					<div class="service-item">
						<div class="service-item-icon">
							<svg
								id="web"
								enable-background="new 0 0 512.418 512.418"
								viewBox="0 0 512.418 512.418"
								xmlns="http://www.w3.org/2000/svg"
								id="web"
							>
								<path
									d="m437.335 75.082c-100.1-100.102-262.136-100.118-362.252 0-100.103 100.102-100.118 262.136 0 362.253 100.1 100.102 262.136 100.117 362.252 0 100.103-100.102 100.117-262.136 0-362.253zm-10.706 325.739c-11.968-10.702-24.77-20.173-38.264-28.335 8.919-30.809 14.203-64.712 15.452-99.954h75.309c-3.405 47.503-21.657 92.064-52.497 128.289zm-393.338-128.289h75.309c1.249 35.242 6.533 69.145 15.452 99.954-13.494 8.162-26.296 17.633-38.264 28.335-30.84-36.225-49.091-80.786-52.497-128.289zm52.498-160.936c11.968 10.702 24.77 20.173 38.264 28.335-8.919 30.809-14.203 64.712-15.452 99.954h-75.31c3.406-47.502 21.657-92.063 52.498-128.289zm154.097 31.709c-26.622-1.904-52.291-8.461-76.088-19.278 13.84-35.639 39.354-78.384 76.088-88.977zm0 32.708v63.873h-98.625c1.13-29.812 5.354-58.439 12.379-84.632 27.043 11.822 56.127 18.882 86.246 20.759zm0 96.519v63.873c-30.119 1.877-59.203 8.937-86.246 20.759-7.025-26.193-11.249-54.82-12.379-84.632zm0 96.581v108.254c-36.732-10.593-62.246-53.333-76.088-88.976 23.797-10.817 49.466-17.374 76.088-19.278zm32.646 0c26.622 1.904 52.291 8.461 76.088 19.278-13.841 35.64-39.354 78.383-76.088 88.976zm0-32.708v-63.873h98.625c-1.13 29.812-5.354 58.439-12.379 84.632-27.043-11.822-56.127-18.882-86.246-20.759zm0-96.519v-63.873c30.119-1.877 59.203-8.937 86.246-20.759 7.025 26.193 11.249 54.82 12.379 84.632zm0-96.581v-108.254c36.734 10.593 62.248 53.338 76.088 88.977-23.797 10.816-49.466 17.373-76.088 19.277zm73.32-91.957c20.895 9.15 40.389 21.557 57.864 36.951-8.318 7.334-17.095 13.984-26.26 19.931-8.139-20.152-18.536-39.736-31.604-56.882zm-210.891 56.882c-9.165-5.947-17.941-12.597-26.26-19.931 17.475-15.394 36.969-27.801 57.864-36.951-13.068 17.148-23.465 36.732-31.604 56.882zm.001 295.958c8.138 20.151 18.537 39.736 31.604 56.882-20.895-9.15-40.389-21.557-57.864-36.951 8.318-7.334 17.095-13.984 26.26-19.931zm242.494 0c9.165 5.947 17.942 12.597 26.26 19.93-17.475 15.394-36.969 27.801-57.864 36.951 13.067-17.144 23.465-36.729 31.604-56.881zm26.362-164.302c-1.249-35.242-6.533-69.146-15.452-99.954 13.494-8.162 26.295-17.633 38.264-28.335 30.84 36.225 49.091 80.786 52.497 128.289z"
								/>
							</svg>
						</div>
						<h2 class="service-item-heading">
							web development
						</h2>
					</div>
				</div>
				<div class="services-container grid-two">
					<div class="service-item">
						<div class="service-item-icon">
							<svg
								viewBox="-3 0 512 512"
								id="e-com"
								xmlns="http://www.w3.org/2000/svg"
							>
								<path
									d="m494.929688 97.148438c-9.492188-10.84375-23.023438-16.816407-38.097657-16.816407h-364.429687l-3.667969-27.5c-3.953125-29.625-24.578125-52.832031-46.957031-52.832031h-26.777344c-8.285156 0-15 6.714844-15 15s6.714844 15 15 15h26.777344c4.8125 0 15.003906 10.175781 17.21875 26.796875l42.9375 322.039063c1.890625 14.195312 9.183594 27.484374 20.535156 37.421874 8.417969 7.371094 18.367188 12.273438 28.789062 14.324219-4.246093 7.808594-6.660156 16.753907-6.660156 26.25 0 30.421875 24.75 55.167969 55.167969 55.167969s55.167969-24.746094 55.167969-55.167969c0-9.0625-2.210938-17.613281-6.101563-25.164062h78.601563c-3.890625 7.550781-6.101563 16.101562-6.101563 25.164062 0 30.421875 24.75 55.167969 55.167969 55.167969s55.167969-24.746094 55.167969-55.167969c0-9.0625-2.210938-17.613281-6.101563-25.164062h39.300782c8.28125 0 15-6.71875 15-15 0-8.285157-6.71875-15-15-15h-302.589844c-14.65625 0-28.671875-12.273438-30.609375-26.800781l-3.136719-23.535157h116.746094c.003906 0 .007812.003907.011718.003907.003907 0 .007813-.003907.015626-.003907h96.367187c.003906 0 .007813.003907.015625.003907.003906 0 .003906-.003907.007812-.003907h83.003907c14.320312 0 28.457031-5.472656 39.808593-15.40625 11.351563-9.9375 18.644532-23.230469 20.535157-37.425781l21.515625-161.367188c1.992187-14.941406-2.136719-29.140624-11.628906-39.984374zm-269.996094 359.683593c0 13.878907-11.289063 25.167969-25.167969 25.167969-13.875 0-25.167969-11.289062-25.167969-25.167969 0-13.875 11.292969-25.164062 25.167969-25.164062 13.878906 0 25.167969 11.289062 25.167969 25.164062zm176.734375 0c0 13.878907-11.289063 25.167969-25.167969 25.167969s-25.167969-11.289062-25.167969-25.167969c0-13.875 11.289063-25.164062 25.167969-25.164062s25.167969 11.289062 25.167969 25.164062zm70.6875-339.921875c3.714843 4.242188 5.300781 10.011719 4.464843 16.253906l-9.019531 67.667969h-102.0625l6.03125-90.5h85.0625c6.300781 0 11.8125 2.335938 15.523438 6.578125zm-213.019531 204.421875-6.035157-90.5h80.371094l-6.035156 90.5zm-8.035157-120.5-6.03125-90.5h96.433594l-6.03125 90.5zm-36.097656-90.5 6.03125 90.5h-112.769531l-12.066406-90.5zm-102.738281 120.5h110.769531l6.035156 90.5h-104.738281zm312.234375 90.5h-66.996094l6.035156-90.5h100.0625l-8.492187 63.703125c-1.9375 14.527344-15.957032 26.796875-30.609375 26.796875zm0 0"
								/>
							</svg>
						</div>
						<h2 class="service-item-heading">
							e-commerce websites
						</h2>
					</div>
					<div class="service-item">
						<div class="service-item-icon">
							<svg
								version="1.1"
								id="erp"
								xmlns="http://www.w3.org/2000/svg"
								xmlns:xlink="http://www.w3.org/1999/xlink"
								x="0px"
								y="0px"
								viewBox="0 0 512 512"
								style="enable-background: new 0 0 512 512;"
								xml:space="preserve"
							>
								<g>
									<g>
										<path
											d="M386.689,304.403c-35.587,0-64.538,28.951-64.538,64.538s28.951,64.538,64.538,64.538
										c35.593,0,64.538-28.951,64.538-64.538S422.276,304.403,386.689,304.403z M386.689,401.21c-17.796,0-32.269-14.473-32.269-32.269
										c0-17.796,14.473-32.269,32.269-32.269c17.796,0,32.269,14.473,32.269,32.269C418.958,386.738,404.485,401.21,386.689,401.21z"
										/>
									</g>
								</g>
								<g>
									<g>
										<path
											d="M166.185,304.403c-35.587,0-64.538,28.951-64.538,64.538s28.951,64.538,64.538,64.538s64.538-28.951,64.538-64.538
										S201.772,304.403,166.185,304.403z M166.185,401.21c-17.796,0-32.269-14.473-32.269-32.269c0-17.796,14.473-32.269,32.269-32.269
										c17.791,0,32.269,14.473,32.269,32.269C198.454,386.738,183.981,401.21,166.185,401.21z"
										/>
									</g>
								</g>
								<g>
									<g>
										<path
											d="M430.15,119.675c-2.743-5.448-8.32-8.885-14.419-8.885h-84.975v32.269h75.025l43.934,87.384l28.838-14.5L430.15,119.675z"
										/>
									</g>
								</g>
								<g>
									<g>
										<rect
											x="216.202"
											y="353.345"
											width="122.084"
											height="32.269"
										/>
									</g>
								</g>
								<g>
									<g>
										<path
											d="M117.781,353.345H61.849c-8.912,0-16.134,7.223-16.134,16.134c0,8.912,7.223,16.134,16.134,16.134h55.933
										c8.912,0,16.134-7.223,16.134-16.134C133.916,360.567,126.693,353.345,117.781,353.345z"
										/>
									</g>
								</g>
								<g>
									<g>
										<path
											d="M508.612,254.709l-31.736-40.874c-3.049-3.937-7.755-6.239-12.741-6.239H346.891V94.655
										c0-8.912-7.223-16.134-16.134-16.134H61.849c-8.912,0-16.134,7.223-16.134,16.134s7.223,16.134,16.134,16.134h252.773v112.941
										c0,8.912,7.223,16.134,16.134,16.134h125.478l23.497,30.268v83.211h-44.639c-8.912,0-16.134,7.223-16.134,16.134
										c0,8.912,7.223,16.134,16.134,16.134h60.773c8.912,0,16.134-7.223,16.135-16.134V264.605
										C512,261.023,510.806,257.538,508.612,254.709z"
										/>
									</g>
								</g>
								<g>
									<g>
										<path
											d="M116.706,271.597H42.487c-8.912,0-16.134,7.223-16.134,16.134c0,8.912,7.223,16.134,16.134,16.134h74.218
										c8.912,0,16.134-7.223,16.134-16.134C132.84,278.82,125.617,271.597,116.706,271.597z"
										/>
									</g>
								</g>
								<g>
									<g>
										<path
											d="M153.815,208.134H16.134C7.223,208.134,0,215.357,0,224.269s7.223,16.134,16.134,16.134h137.681
										c8.912,0,16.134-7.223,16.134-16.134S162.727,208.134,153.815,208.134z"
										/>
									</g>
								</g>
								<g>
									<g>
										<path
											d="M180.168,144.672H42.487c-8.912,0-16.134,7.223-16.134,16.134c0,8.912,7.223,16.134,16.134,16.134h137.681
										c8.912,0,16.134-7.223,16.134-16.134C196.303,151.895,189.08,144.672,180.168,144.672z"
										/>
									</g>
								</g>
								<g></g>
								<g></g>
								<g></g>
								<g></g>
								<g></g>
								<g></g>
								<g></g>
								<g></g>
								<g></g>
								<g></g>
								<g></g>
								<g></g>
								<g></g>
								<g></g>
								<g></g>
							</svg>
						</div>
						<h2 class="service-item-heading">
							ERP SYSTEM
						</h2>
					</div>
				</div>
			</div>
		</section>

		<section class="section-feature">
			<div class="feature-container">
				<div class="feature-text wow slideInLeft">
					<h2 class="feature-text_header">
						Booting You E-Commerce Website
					</h2>
					<div class="feature-text_par">
						<ul>
							<li>Lorem ipsum, dolor sit amet</li>
							<li>consectetur adipisicing elit.</li>
							<li>Architecto qui autem voluptatum</li>
						</ul>
					</div>
				</div>
				<div class="feature-image wow slideInRight">
					<img
						src="./assets/images/innovation_.png"
						id="innovation"
						alt="ex"
					/>
				</div>
			</div>
		</section>

		<section class="section-feature odd-section">
			<div class="feature-container" id="odd-col">
				<div class="feature-image wow slideInLeft">
					<img src="./assets/images/web_developer.png" alt="ex" />
				</div>

				<div class="feature-text wow slideInRight">
					<h2 class="feature-text_header">
						Booting You E-Commerce Website
					</h2>
					<div class="feature-text_par">
						<ul>
							<li>Lorem ipsum, dolor sit amet</li>
							<li>consectetur adipisicing elit.</li>
							<li>Architecto qui autem voluptatum</li>
						</ul>
					</div>
				</div>
			</div>
		</section>

		<section class="section-testimoials">
			<h2 class="section-header font-white">What they said about us</h2>
			<div class="glider-contain wow fadeIn">
				<div class="glider">
					<div class="testimonial_item">
						<div class="testimonial_image">
							<img src="./assets/images/0.jpg" alt="zeyad" />
						</div>
						<h3 class="testimonial_name">Zeyad Mohamed</h3>
						<p class="testimonial_par">
							Lorem ipsum dolor sit amet consectetur adipisicing
							elit. Laboriosam excepturi vero quas fuga tempore
							voluptate natus deserunt culpa voluptatibus magni.
							Atque temporibus provident vitae
						</p>
					</div>
					<div class="testimonial_item">
						<img
							class="testimonial_image"
							src="./assets/images/1.jpg"
							alt="Ahamed galal"
						/>
						<h3 class="testimonial_name">Ahmed Galal</h3>
						<p class="testimonial_par">
							Lorem ipsum dolor sit amet consectetur adipisicing
							elit. Autem at esse temporibus quas illum commodi
							minima, veritatis quisquam a obcaecati aspernatur
							fugit quae possimus omnis
						</p>
					</div>
					<div class="testimonial_item">
						<img
							class="testimonial_image"
							src="./assets/images/img.jpg"
							alt="masry araby"
						/>
						<h3 class="testimonial_name">Masry araby</h3>
						<p class="testimonial_par">
							Lorem ipsum dolor sit amet consectetur adipisicing
							elit. Consequuntur quibusdam ut explicabo et maxime
							quaerat, error, facilis excepturi placeat minus eos
						</p>
					</div>
					<div class="testimonial_item">
						<img
							class="testimonial_image"
							src="./assets/images/0.jpg"
							alt="Omar Gaber"
						/>
						<h3 class="testimonial_name">Omar Gaber</h3>
						<p class="testimonial_par">
							"Lorem ipsum dolor sit amet consectetur adipisicing
							elit. Libero omnis assumenda a inventore tenetur.
							Explicabo eius ut totam officia expedita aliquid
							atque perspiciatis"
						</p>
					</div>
				</div>

				<button aria-label="Previous" class="glider-prev">«</button>
				<button aria-label="Next" class="glider-next">»</button>
				<div role="tablist" class="dots"></div>
			</div>
		</section>

		<section class="section-ourwork">
			<h2 class="section-header">HOW WE work</h2>
			<div class="ourwork_container">
				<div class="ourwork wow fadeInDown">
					<div class="ourwork_icon">
						<i class="far fa-dot-circle"></i>
					</div>
					<div class="ourwork_heading">WE WORK FAST</div>
					<div class="ourwork_par">
						Lorem ipsum dolor sit amet consectetur adipisicing elit.
						Repellat doloribus quos molestias impedit laboriosam aut
						delectus doloribus quos molestias impedit laboriosam aut
					</div>
				</div>
				<div class="ourwork wow fadeInDown">
					<div class="ourwork_icon">
						<i class="fas fa-shipping-fast"></i>
					</div>
					<div class="ourwork_heading">WE WORK FAST</div>
					<div class="ourwork_par">
						Lorem ipsum dolor sit amet consectetur adipisicing elit.
						Repellat doloribus quos molestias impedit laboriosam aut
						delectus doloribus quos molestias impedit laboriosam aut
					</div>
				</div>
				<div class="ourwork wow fadeInDown">
					<div class="ourwork_icon">
						<i class="fas fa-lightbulb"></i>
					</div>
					<div class="ourwork_heading">WE WORK FAST</div>
					<div class="ourwork_par">
						Lorem ipsum dolor sit amet consectetur adipisicing elit.
						Repellat doloribus quos molestias impedit laboriosam aut
						delectus doloribus quos molestias impedit laboriosam aut
					</div>
				</div>
				<div class="ourwork wow fadeInDown">
					<div class="ourwork_icon">
						<i class="fas fa-bullhorn"></i>
					</div>
					<div class="ourwork_heading">WE WORK FAST</div>
					<div class="ourwork_par">
						Lorem ipsum dolor sit amet consectetur adipisicing elit.
						Repellat doloribus quos molestias impedit laboriosam aut
						delectus doloribus quos molestias impedit laboriosam aut
					</div>
				</div>
			</div>
		</section>

		<section class="contact odd-section">
			<h2 class="section-header">CONTACT us</h2>

			<div class="contact_container">
				<div class="contact_map">
					<div
						id="map"
						style="width: 400px; height: 400px; background: grey;"
					>
						<script>
							function myMap() {
								var mapOptions = {
									center: new google.maps.LatLng(51.5, -0.12),
									zoom: 10,
									mapTypeId: google.maps.MapTypeId.HYBRID,
								};
								var map = new google.maps.Map(
									document.getElementById("map"),
									mapOptions
								);
							}
						</script>
						<script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyDWweLTl5rXcXwyb_DhB2cByhgyspmrT8Q&callback=myMap"></script>
					</div>
				</div>

				<div class="contact_form">
					<form action="">
						<input
							type="text"
							placeholder="email"
							name="email"
							id="email"
						/>
						<input
							type="text"
							placeholder="name"
							name="name"
							id="name"
						/>
						<input
							type="text"
							placeholder="your phone number"
							name="phone"
							id="phone"
						/>
						<textarea
							name="messahe"
							id="message"
							cols="30"
							rows="10"
							placeholder="write your message here"
						></textarea>
					</form>
				</div>
			</div>
		</section>

		<section class="portofolio">
			<h2 class="section-header">WE LOVED to wokring with</h2>
			<div class="project-container">
				<div class="project">
					<img src="./assets/images/profolio/g1.jpg" alt="techzone" />
					<p>Aziz App</p>
					<span
						>Lorem ipsum dolor sit, amet consectetur adipisicing
						elit. Enim obcaecati accusamus
					</span>
					<button>VISIT</button>
				</div>
				<div class="project">
					<img src="./assets/images/profolio/g2.jpg" alt="techzone" />
					<p>Matego</p>
					<span
						>Lorem ipsum dolor sit, amet consectetur adipisicing
						elit. Enim obcaecati accusamus
					</span>
					<button>VISIT</button>
				</div>
				<div class="project">
					<img src="./assets/images/profolio/g3.jpg" alt="techzone" />
					<p>maysure</p>
					<span
						>Lorem ipsum dolor sit, amet consectetur adipisicing
						elit. Enim obcaecati accusamus
					</span>
					<button>VISIT</button>
				</div>
				<div class="project">
					<img src="./assets/images/profolio/g4.jpg" alt="techzone" />
					<p>Lunch box</p>
					<span
						>Lorem ipsum dolor sit, amet consectetur adipisicing
						elit. Enim obcaecati accusamus
					</span>
					<button>VISIT</button>
				</div>
				<div class="project">
					<img src="./assets/images/profolio/g5.jpg" alt="techzone" />
					<p>Al-Ahmed Realstate</p>
					<span
						>Lorem ipsum dolor sit, amet consectetur adipisicing
						elit. Enim obcaecati accusamus
					</span>
					<button>VISIT</button>
				</div>
				<div class="project">
					<img src="./assets/images/profolio/g6.jpg" alt="techzone" />
					<p>Tech Cube</p>
					<span
						>Lorem ipsum dolor sit, amet consectetur adipisicing
						elit. Enim obcaecati accusamus
					</span>
					<button>VISIT</button>
				</div>
			</div>
		</section>

		<footer class="footer-section">
			<div class="footer-container">
				<div class="footer_col footer_logo">
					<img
						src="./assets/images/techzone.png"
						alt="Techzonelabs"
					/>
					<p>
						Aiming to be your Tech-Arm and success partner, writing
						memorable stories along the journey
					</p>
				</div>
				<div class="footer_col footer_contact">
					<h2>Contact</h2>

					<ul>
						<li><i></i> Alex-Cairo-Kuwait.</li>
						<li><i></i> +(20) 1060 530 287</li>
						<li><i></i> ahmad@techzonelabs.com</li>
					</ul>
				</div>
				<div class="footer_col footer_services">
					<h2>Services</h2>
					<ul>
						<li>Lorem ipsum dolor sit</li>
						<li>consectetur adipisicing elit</li>
						<li>Quo, magni deserunt! Nam</li>
						<li>non impedit sapiente quae</li>
					</ul>
				</div>
				<div class="footer_col footer_about">
					<h2>About</h2>
					<ul>
						<li>our team</li>
						<li>Join us</li>
						<li>Contact US</li>
						<li>Our projects</li>
					</ul>
				</div>
				<div class="footer_col footer_blog">
					<h2>Blog</h2>
					<p>subscribe to our news letter</p>
				</div>
			</div>
		</footer>
		<script src="./assets/vendor/WOW-master/dist/wow.js"></script>
		<script defer src="./assets/js/app.js"></script>
	</body>
</html>
